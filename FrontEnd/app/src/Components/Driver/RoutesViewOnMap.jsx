...
import './RoutesViewOnMap.css';


const RoutesViewOnMap=()=>{
    //Params
    ...
    const [routeControl, setRouteControl] = useState(null);    // URL
    ...
    const getOrdersPlacedByDriver = async() =>{
        setIsLoading(true);
        try{
            ...
            //const orderPlacementIds = responseOfOrdersPlacedByDriver.data.map(d => d.Id)
            const orderPlacementIds =responseRoutes.data.TravelinSequency.split('-').map(Number)
            console.log("SEARCH--",orderPlacementIds)
            ...
            const sortedTrackings = orderPlacementIds
              .map(id=> filteredTrackings.find(tracking => tracking.OrderPlacementId === id))
              .filter(tracking => tracking !== undefined);

            console.log("sortedTrackings",sortedTrackings)

            setOrders(sortedTrackings);
    ...
    const displayTrackingPins = useCallback(() => {
        ...
        orders.forEach((tracking, index) => {    
          console.log("Indexs",tracking.Id)
          ...
          if (deliveryCoords) {
            const deliveryIcon = L.divIcon({
              ...
              html: `<i class="fas fa-home"></i><span class="marker-label">#${index+1} </span>`,
              iconSize: [40, 40],
              iconAnchor: [20, 20]
            });
    ...
    return(
        <div className="RoutesViewOnMap">
                    ...
                    <div className="map-container">
                        <div id="map" style={{ height: '100%', width: '100%', position:'static' }} className="map-routes"></div>            
...
export default RoutesViewOnMap;